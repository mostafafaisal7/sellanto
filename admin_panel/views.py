...
# ─────────────────────────────────────────────────────────────────────
# Email Settings (admin panel — dynamic SMTP configuration)
# ─────────────────────────────────────────────────────────────────────
...
def get_email_settings(request):
    """GET — return current email settings (password masked)."""
    from accounts.models import SiteConfiguration
    from django.conf import settings

    data = {
        'email_host': SiteConfiguration.get('email_host') or getattr(settings, 'EMAIL_HOST', 'smtp.gmail.com'),
        'email_port': SiteConfiguration.get('email_port') or str(getattr(settings, 'EMAIL_PORT', 587)),
        'email_use_tls': SiteConfiguration.get('email_use_tls') or str(getattr(settings, 'EMAIL_USE_TLS', True)),
        'email_host_user': SiteConfiguration.get('email_host_user') or getattr(settings, 'EMAIL_HOST_USER', ''),
        'email_host_password': '••••••••' if (SiteConfiguration.get('email_host_password') or getattr(settings, 'EMAIL_HOST_PASSWORD', '')) else '',
        'email_default_from': SiteConfiguration.get('email_default_from') or getattr(settings, 'DEFAULT_FROM_EMAIL', ''),
        'email_admin_notification': SiteConfiguration.get('email_admin_notification') or getattr(settings, 'ADMIN_NOTIFICATION_EMAIL', ''),
    }
...
def update_email_settings(request):
    """POST — save email settings to SiteConfiguration."""
    from accounts.models import SiteConfiguration
    ...
    for key in EMAIL_SETTING_KEYS:
        ...
        if key == 'email_host_password' and value == '••••••••':
            continue
        SiteConfiguration.set(key, value, descriptions.get(key, ''))
...
def test_email_settings(request):
    """POST — send a test email using the currently saved settings."""
    if request.method != 'POST':
