...
| 4 | `instagram_content_publish` | — | — | ⚠️ test not done | [04-instagram_content_publish.md](04-instagram_content_publish.md) |
...
| 8 | `instagram_basic` | — | — | ⚠️ test not done | [08-instagram_basic.md](08-instagram_basic.md) |
| 9 | `pages_messaging` (Messenger) | needs adding | — | 🔴 MISSING from scopes | [09-messaging-whatsapp-and-messenger.md](09-messaging-whatsapp-and-messenger.md) |
...
- **`instagram_content_publish`** — ✅ **already covered** (organic photo + video/reel
  publish to IG main feed, built + wired + in production). No build needed; publish
  one IG post to fire the call. See file 04.
...
- **`ads_management`** — ✅ **already covered** (full create + manage + insights built
  ...
  file 07.
- **`instagram_basic`** — ✅ **already covered** (IG profile + media read; foundational
  scope, in FB_SCOPES + test-calls cmd). No build needed. See file 08.
- **Messenger / WhatsApp** (file 09):
  ...
  - **FB Messenger** — ✅ full feature (`messenger_bot` app: webhook + `/me/messages`).
    🔴 **`pages_messaging` is MISSING from `FB_SCOPES`** — works for admin in dev tier
    but will FAIL for real users. Action: add `pages_messaging` to scopes + test call +
